...
[] spawn {
	...
	if(isServer) then {
		...
		["OT_civilian_cleanup_crew", "time > OT_cleanup_civilian_loop","
			...
			{
				if (side group _x isEqualTo civilian && {!(isPlayer _x)} && {!(_x getVariable [""shopcheck"",false])} && {!(_x getVariable [""priest"",false])} && { ({side _x isEqualTo civilian} count ((getPos _x) nearObjects [""CAManBase"",150])) > round(150*OT_spawnCivPercentage) } ) then {
